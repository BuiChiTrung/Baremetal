...
void spi1_transmit(uint8_t *data, uint32_t size) {
  ...
  // Wait until last byte is transmitted (TXE = 1) and SPI is not busy (BSY = 0)
  while ((SPI1->SR & SR_BSY))
    ;

  /*
    After the master transmits data through the Data reg, data reg captures the
    data received from the slave. To ensure incoming data is properly processed,
    we read the Data Reg, even if we don’t need the value. This read operation
    automatically clears the OVR flag. It’s also advisable to read the Status
